...
plt.plot([bt['Nodes'] for bt in backoff_times], [bt['Old Backoff Time'] for bt in backoff_times], 'b-', linewidth=1.5, label='Backoff time calculated using traditional method')
plt.plot([bt['Nodes'] for bt in backoff_times], [bt['New Backoff Time'] for bt in backoff_times], 'r-', linewidth=1.5, label='Backoff time calculated using proposed method')
...
plt.plot([cc['Nodes'] for cc in collision_counts], [cc['Collision Count'] for cc in collision_counts], 'g-', linewidth=1.5, label='Collision Count calculated using proposed method')
plt.scatter([cc['Nodes'] for cc in collision_counts], [cc['Collision Count'] for cc in collision_counts], color='g', s=50)  # Add markers for each data point
plt.xlabel('Number of Nodes')
...
df_backoff = pd.DataFrame(backoff_times)
df_collision = pd.DataFrame(collision_counts)
df_comparison = pd.DataFrame(collision_comparisons)

# Display the DataFrames
print("Backoff Time Comparison:")
print(df_backoff)
print("\nCollision Count using the New Algorithm:")
print(df_collision)
print("\nCollision Count using the Old Algorithm:")
print(df_comparison)
